...
This section matters because discoverability grows only when technical, content, commercial, AI discoverability, baseline are handled as an execution system rather than isolated tasks.

## When to Use

Use it for all sites before launch or recovery. Prioritize it early if the site is launching, relaunching, or recovering from a traffic or trust problem.
...
3. Break the work into actions around technical, content, commercial, AI discoverability, baseline.
...
Task: produce an action plan for Audit with priorities, QA steps, risks, and expected outputs.
...
Example AI Agency applies Audit first to audit, GEO service pages, and branded AI prompts before scaling to the full blog.
