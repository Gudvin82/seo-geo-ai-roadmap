...
Этот раздел важен, потому что discoverability растет только тогда, когда типы страниц, логика URL, хабы, перелинковка управляются как единая система исполнения, а не как набор разрозненных задач.

## Когда Использовать

Используйте его для сценариев: новые сайты, перезапуски и multi-service структуры. Ставьте в приоритет в начале, если сайт запускается, перезапускается или выходит из просадки трафика и доверия.
...
3. Разбейте работу на действия вокруг: типы страниц, логика URL, хабы, перелинковка.
...
Задача: составь action plan для раздела Архитектура сайта с приоритетами, QA-шагами, рисками и expected outputs.
...
Пример AI Агентство сначала применяет раздел Архитектура сайта к аудиту, GEO-страницам услуг и брендовым AI-промптам, а уже потом масштабирует на весь блог.
