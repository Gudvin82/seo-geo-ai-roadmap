...
Этот раздел важен, потому что discoverability растет только тогда, когда консоль, аналитика, краулеры, AI-ассистенты и governance tools управляются как единая система исполнения, а не как набор разрозненных задач.

## Когда Использовать

Используйте его для сценариев: все команды, строящие repeatable stack. Ставьте в приоритет в начале, если сайт запускается, перезапускается или выходит из просадки трафика и доверия.
...
2. Разбейте работу на действия вокруг: консоль, аналитика, краулеры, AI-ассистенты и governance tools.
...
Задача: составь action plan для раздела Инструменты с приоритетами, QA-шагами, рисками и expected outputs.
...
Пример AI Агентство сначала применяет раздел Инструменты к аудиту, GEO-страницам услуг и брендовым AI-промптам, а уже потом масштабирует на весь блог.
