...
Этот раздел важен, потому что discoverability растет только тогда, когда техника, контент, коммерция, AI discoverability, baseline управляются как единая система исполнения, а не как набор разрозненных задач.

## Когда Использовать

Используйте его для сценариев: любой сайт перед запуском или восстановлением. Ставьте в приоритет в начале, если сайт запускается, перезапускается или выходит из просадки трафика и доверия.
...
3. Разбейте работу на действия вокруг: техника, контент, коммерция, AI discoverability, baseline.
...
Задача: составь action plan для раздела Аудит с приоритетами, QA-шагами, рисками и expected outputs.
...
Пример AI Агентство сначала применяет раздел Аудит к аудиту, GEO-страницам услуг и брендовым AI-промптам, а уже потом масштабирует на весь блог.
