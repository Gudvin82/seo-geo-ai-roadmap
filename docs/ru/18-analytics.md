...
Этот раздел важен, потому что discoverability растет только тогда, когда GA4, GSC, Метрика, дашборды, аннотации, AI-мониторинг управляются как единая система исполнения, а не как набор разрозненных задач.

## Когда Использовать

Используйте его для сценариев: команды, где нужен повторяемый контур отчетности. Ставьте в приоритет в начале, если сайт запускается, перезапускается или выходит из просадки трафика и доверия.
...
3. Разбейте работу на действия вокруг: GA4, GSC, Метрика, дашборды, аннотации, AI-мониторинг.
...
Задача: составь action plan для раздела Аналитика и мониторинг с приоритетами, QA-шагами, рисками и expected outputs.
...
Пример AI Агентство сначала применяет раздел Аналитика и мониторинг к аудиту, GEO-страницам услуг и брендовым AI-промптам, а уже потом масштабирует на весь блог.
