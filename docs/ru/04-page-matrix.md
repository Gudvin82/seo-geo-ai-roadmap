...
Этот раздел важен, потому что discoverability растет только тогда, когда одна страница = один интент = один CTA = один owner управляются как единая система исполнения, а не как набор разрозненных задач.

## Когда Использовать

Используйте его для сценариев: сервисы, SaaS, агентства и маркетплейсы. Ставьте в приоритет в начале, если сайт запускается, перезапускается или выходит из просадки трафика и доверия.
...
3. Разбейте работу на действия вокруг: одна страница = один интент = один CTA = один owner.
...
Задача: составь action plan для раздела Матрица страниц с приоритетами, QA-шагами, рисками и expected outputs.
...
Пример AI Агентство сначала применяет раздел Матрица страниц к аудиту, GEO-страницам услуг и брендовым AI-промптам, а уже потом масштабирует на весь блог.
